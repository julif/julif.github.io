/* { 
    titulo: ''  ,
    id: 'blog_016',
    thumbnail: '', 
    fecha: [2020, 12, 1],
    snippet: '' ,
    custom_link: '' ,
    entrada: ''    
  }, */


var blog_entrys = [
  
    
  { 
    titulo: 'Proyecto deckbuilder'  ,
    id: 'blog_019',
    fecha: [2021, 3, 3],
    custom_link: 'blog_entry_019.html',
    // thumbnail: 'media/thumb_deckbuilder.png',
    thumbnail:'media/20210310_052123.png',
    snippet: ' un juego de cartas para navegador' ,
    etiquetas: ["desarollo web", "desarollo de videojuegos"],
    entrada:  ' ... '    
  },
 
  { 
    titulo: 'art vs artist 2020'  ,
    id: 'blog_018',
    fecha: [2020, 10, 13],
    custom_link: ' ',
    snippet: 'art vs artist' ,
    etiquetas: ["art vs artist"],
    entrada:  '<img src="../media/art_vs_artist_2020.jpg"></img> <p>#artvsartist #artvsartist2020 #julif </p>'    
  },
  { 
    titulo: 'Dress-up de Bunny'  ,
    id: 'blog_017',
    fecha: [2020, 9, 30],
    custom_link: 'blog_entry_017.html',
     thumbnail: 'media/017.png',
    snippet: 'un dress-up web para echo con html y javascript' ,
    etiquetas: ["desarollo web", "desarollo de videojuegos","dress-up"],
    entrada:  ' ... '    
  },
  { 
    titulo: 'formato helltaker'  ,
    id: 'blog_016',
    fecha: [2020, 7, 2],
    custom_link: ' ',
    etiquetas: ["helltaker"],
    snippet: 'me gusta el formato de de helltaker para usar en novelas visuales' ,
    entrada:  '<img src="../media/Eabv0LqXsAAfdDV.jpg"></img><p>me gusta el formato de de helltaker para usar en novelas visuales</p>'    
  },
  { 
    titulo: 'Nuevo dragon'  ,
    id: 'blog_015',
    thumbnail: 'media/dragon6.png', 
    fecha: [2020, 3, 11],
    etiquetas: ["dragon", "re-do"],
    snippet: 'me molestaba mucho el horrible intento de personaje que dibuje frente a este dragon hace tantos años' ,
    entrada: '<img src="../media/cxcxcxcxb-1024x1013.png"> <p>me molestaba mucho el horrible intento de personaje que dibuje frente a este dragon hace tantos años, asi que lo saque de ese feo dibujo y lo actualize un poco.</p><img src="../media/jinete_de_dragon_by_julif_art_d7sklm1-fullview-1.jpg"> <img src="../media/dragon5.png"></img><img src="../media/dragon6.png"></img>'  
  },

  
  { 
    titulo: 'practica con julif chibi'  ,
    id: 'blog_014',
    thumbnail: 'media/ezgif-2-61607a0c764f.gif',
    fecha: [2020, 3, 11],
    etiquetas: ["julif", "chibi"],
    snippet: 'otro julif chibi' ,
    entrada: '<p>me parece que este estilo es ideal para animatics simples o videos de storytelling.</p> <img src="../media/Untitled-1fdfd-1024x576.png"></img><img src="../media/cdcdcdcd-1024x576.png"></img><img src="../media/ezgif-2-61607a0c764f.gif"></img>'    
  },
  { 
    titulo: 'Chibi Tracer'  ,
    id: 'blog_013',
    fecha: [2020, 2, 18],
    etiquetas: ["overwatch", "Tracer", "fan-art", "chibi" ],
    snippet: 'otro fan-art de overwatch' ,
    entrada: '<img src="../media/tracer.jpg"></img><img src="../media/tracer.gif"></img>'    
  },
  { 
    titulo: 'Julif'  ,
    id: 'blog_012',
    fecha: [2020, 2, 18],
    etiquetas: ["julif", "chibi"],
    snippet: '...' ,
    entrada: '<img src="../media/julif2.gif">'    
  },
  { 
    titulo: 'Portada de shacta'  ,
    id: 'blog_011',
    fecha: [2020, 2, 9],
    etiquetas: ["shacta"],
    snippet: 'La vieja portada que hice para un webcomic interactivo que nunca termine' ,
    entrada: '<img src="../media/capitulo1fondo.png"><p>La vieja portada que hice para un webcomic interactivo que nunca termine</p>'    
  },
  { 
    titulo: 'julif chibi 2008-2019'  ,
    id: 'blog_010',
    thumbnail: 'media/dffdfdfd-1024x576.png',  
    fecha: [2020, 2, 7],
    etiquetas: ["julif", "chibi"],
    snippet: 'me gusta cada tanto ver mi propio progreso' ,
    entrada: '<img src="../media/dffdfdfd-1024x576.png"></img><p>me gusta cada tanto ver mi propio progreso</p>'    
  },
  { 
    titulo: 'prueba de animacion'  ,
    id: 'blog_009',
    fecha: [2020, 1, 1],
    etiquetas: ["julif"],
    snippet: 'una vieja prueba de animación que encontré por ahí… debería actualizarla.' ,
    entrada: '<img src="../media/julif.gif"><p>una vieja prueba de animación que encontré por ahí… debería actualizarla.</p>'    
  }, 
  { 
    titulo: 'dragologia',
    id: 'blog_008',
    thumbnail: 'media/dragon3.png',
    fecha: [2020, 1, 13],
    etiquetas: ["dragon"],
    snippet: 'algunos de mis dragones' ,
    entrada: ' <p>algunos de mis dragones</p><img src="../media/dragon1.png"></img> <img src="../media/dragon2.png"></img><img src="../media/dragon3.png"></img><img src="../media/dragon4.jpg"></img>'    
   
  },
  { 
    titulo: 'Que llueva lo que tenga que llover'  ,
    id: 'blog_007',
    thumbnail: 'media/phara.png' ,
    fecha: [2019, 8, 21],
    etiquetas: ["overwatch", "pharah", "fan-art"],
    snippet: 'esta linea aparece en el estribillo de una de sus canciones' ,
    entrada: '<img src="../media/phara.png"></img><p>Irina indigo ( la voz en latino de pharah ) es cantante, y esta linea aparece en el estribillo de una de sus canciones… es simplemente demasiado perfecto.</p><p>Link de la cancion: <a href="https://www.youtube.com/watch?v=U-4Q8_9BfNM&ab_channel=IrinaIndigo">https://youtu.be/U-4Q8_9BfNM </a></p>'    
  },
  { 
    titulo: 'Shacta'  ,
    id: 'blog_006',
    thumbnail: 'media/shacta3.png' ,
    fecha: [2019, 8, 20],
    etiquetas: ["shacta", "chibi"],
    snippet: 'Me gusta dibujar a shacta en este estilo.' ,
    entrada: '<img src="../media/shacta1.png"></img><img src="../media/shacta2.png"></img><img src="../media/shacta3.png"></img><p>Me gusta dibujar a shacta en este estilo.</p> '    
  },
  { 
    titulo: 'Julif'  ,
    id: 'blog_005',
    thumbnail: 'media/hh2-1024x576.png' ,
    fecha: [2019, 8, 13],
    etiquetas: ["julif", "chibi"],
    snippet: 'me encanta como se ve julif en este estilo simplificado y con vectores.' ,
    entrada: '<img src="../media/hh2-1024x576.png"></img> <p> me encanta como se ve julif en este estilo simplificado y con vectores.</p>'    
  },
  { 
    titulo: 'sexy Julif'  ,
    id: 'blog_004',
    fecha: [2019, 7, 16],
    etiquetas: ["sexy", "julif"],
    snippet: 'Lo siento, tenia que hacerlo.' ,
    entrada: '<figure class="wp-block-video"><video controls="" src="../media/julif_sexy.mp4"></video></figure><p>Lo siento, tenia que hacerlo.</p>'    
  },
  { 
    titulo: 'art vs artist 2019'  ,
    id: 'blog_003',
    fecha: [2019, 7, 15],
    etiquetas: ["art vs artist"],
    snippet: 'art vs artist 2019' ,
    entrada: '<img src="../media/art_vs_artist_2019.jpg"></img> <p>#artvsartist #artvsartist2019 #julif </p>'  
  },
  {
    titulo: 'ko-fi',
    id: 'blog_002', 
    thumbnail: 'media/ko-fi.png' ,
    etiquetas: ["julif", "chibi","ko-fi"],
    snippet: 'alfin lo hize, cree una cuenta de ko-fi  ' ,
    entrada: '<img src="../media/ko-fi.png"></img><p>alfin lo hize, cree una cuenta de ko-fi.</p><p>admito que aun no se muy bien como funciona ko-fi, varias personas me lo recomendaron para darle la opcion a la gente de hacer una pequeña donacion como muestra de su  apoyo hacia mi trabajo .</p><p style="text-align:center"><a href="https://ko-fi.com/juli22f">kofi.com/juli22f</a></p>', 
    fecha: [2019, 7, 13]
  },
  { 
    titulo: 'nuevo blog',
    id: 'blog_001', 
    etiquetas: ["blog"],
    snippet: 'Todo final es un nuevo comienzo y esta no es la exepcion.' ,
    entrada: '<blockquote class="wp-block-quote"><p>Todo final es un nuevo comienzo y esta no es la exepcion.</p><cite>Julian filardi 1990-2030</cite></blockquote><h2>¿ que mierda paso?</h2><p>Si bien soy conciente acerca de los rumores  que decian que la CIA cerro mi antiguo blog por mera envidia, la verdad es que  hace algunos meses  <a href="http://Www.hostiguer.com">hostinguer</a>,la plataforma  donde alojaba mi blog descontinuo su version gratuita forzandome a pagar mi propio dominio personal.</p><h2>Julif.com</h2><p> La verdad es que relmente pagar por mi propio dominio no era un problema, ya que llevaba tiempo queriendo desacerme de el subdominio que se adjunto al nombre de mi pagina web como consecuencia de su estado no premiun.</p><p>Si dependiera de mi, esta  pagina seria <a href="http://Www.julif.com">julif.com</a> en lugar de <a href="http://Www.Julifweb.com">julifweb</a>, aunque admito que ambas son mejores que el antiguo  <a href="http://Www.julif.besaba.com">julif.besaba.com</a> ( si, besaba era mi subdominio anterior).</p><p>Pero al parecer otra persona consigio el dominio de mi sueño antes que yo, y no solo eso sino que la persona que compro mi dominio lo tiene olvidado y al parecer no planea hacer nada con el, asi que quien sabe… quizas en el futuro pueda ser dueño de julif.com.</p><h2>¿Y ahora que?</h2><p>Bien ya tengo sitio web nuevo, ahora la cuestion es que voy a hacer con el. Mi idea es comenzar con un blog simple y mixto de mis proyectos tanto de arte como de desarrollo web para eventualmente cumplir con estas ideas:</p><ul><li>Un blog para mis cosas en general, proyectos dibujos y demas.</li><li>Un devblog para tener registro de todo lo que aprenda de javascript, wordpress y desarollo web en general.</li><li>Una seccion donde pueda alojar mi webapps y sus proyectos y demos.</li><li>Un portfolio para poder demostrar lo que se hacer.</li><li>Una landing page para organizar todo lo anterior.</li><li>Crear una seccion o pagina de los dragones de julif (opcional).</li></ul><p>O almenos ese es el plan.</p><p>Estoy muy emocionado de alfin poder compartir todo lo que se hacer en un espacio diseñado por mi.</p>', 
    fecha: [2019, 3, 3]
  
  }
]; 
    
